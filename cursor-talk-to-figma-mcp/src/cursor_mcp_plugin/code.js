...
const state = {
  ...
  channelName: "figma-mcp-default", // Fixed dedicated channel name
};
...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "export_variable_collections":
      return await exportVariableCollections();
    case "set_text_style":
      return await setTextStyle(params);
    case "set_effect_style":
      return await setEffectStyle(params);
    default:
...
// --- Variable extraction helpers for selection ---
...
// Apply a Figma text style (by styleId) to a node.
// - If the node is a TextNode, apply directly.
// - If the node has children, apply to all descendant TextNodes.
async function setTextStyle(params) {
  const nodeId = params && params.nodeId;
  const styleId = params && params.styleId;
  ...
  async function applyToNode(n) {
    if (n.type === "TEXT") {
      const name = (n.name || "").toLowerCase();
      // Heuristic: skip icon text layers (often named "icon")
...
// Apply a Figma effect style (by styleId) to a node.
// Effect styles include shadows, blurs, etc.
async function setEffectStyle(params) {
  const nodeId = params && params.nodeId;
  const styleId = params && params.styleId;
  ...
  // Apply effect style to the node
  // Note: effectStyleId is available on nodes that support effects
  if (typeof node.setEffectStyleIdAsync === "function") {
    await node.setEffectStyleIdAsync(styleId);
  } else {
    throw new Error("This node type does not support effect styles");
  }
...
// Export variable collections + variables from the current file.
// This is used by the MCP host to build a design-system context.
async function exportVariableCollections() {
  // Use async APIs to support dynamic-page documentAccess restrictions
  ...
  return collections.map((c) => ({
    ...
    variables: varsByCollection[c.id] || [],
  }));
}
...
async function setVariableBinding(params) {
  const { nodeId, propertyName, variableId, variableName, collectionId } =
    params || {};
  ...
  // Prefer explicit variableId if provided
  if (variableId) {
    ...
    // Resolve by name within the optional collectionId
    const allVariables = figma.variables.getLocalVariables();
    const matches = allVariables.filter((v) => {
      if (v.name !== variableName) return false;
      if (collectionId && v.variableCollectionId !== collectionId) return false;
      return true;
    });

    if (!matches.length) {
      throw new Error(
        `Variable not found with name "${variableName}"` +
          (collectionId ? ` in collection ${collectionId}` : "")
      );
    }

    // If multiple match, take the first – callers can refine collectionId if needed
    variable = matches[0];
  }

  // Figma API: bind variable to a property on the node
  // For fills and strokes, Figma requires variable bindings on paints directly.
  ...
  if (propertyName === "fills" || propertyName === "strokes") {
    ...
    if (typeof node.setBoundVariable !== "function") {
      throw new Error("This node type does not support scalar variable bindings");
    }
...
async function extractVariablesFromNode(node) {
  const out = [];
  const seen = new Set();
  const anyNode = node;
  const boundVars = anyNode.boundVariables || null;

  if (!boundVars) return out;

  for (const [propName, value] of Object.entries(boundVars)) {
    const entries = Array.isArray(value) ? value : [value];

    for (const entry of entries) {
      // Use async API to satisfy dynamic-page access restrictions
      ...
      if (!seen.has(variable.id)) {
        ...
        const existing = out.find((v) => v.id === variable.id);
        if (existing && !existing.props.includes(propName)) {
          existing.props.push(propName);
        }
      }
    }
  }
...
async function getSelectionVariables() {
  ...
  const allNodes = collectNodes(selection);

  const result = [];

  for (const node of allNodes) {
    const variables = await extractVariablesFromNode(node);
